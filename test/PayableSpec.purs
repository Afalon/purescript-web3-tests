...
import Network.Ethereum.Web3 (ChainCursor(..), Ether, EventAction(..), Shannon, Value, _from, _to, _value, convert, defaultTransactionOptions, event, eventFilter, mkValue, runWeb3, uIntNFromBigNumber, unUIntN)
...
payableSpec =
  describe "interacting with a PayableTest contract" do
    it "can send the right amount of wei" $ do
      ...
      resp <- map (unsafePartial fromRight) <<< runWeb3 httpP $ PayableTest.seeContent txOptions
